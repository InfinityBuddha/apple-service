                ...
                <div className={s.wrapper}>
                    <div className={cx(`${s.itemGroup} ${this.state.active === 'Ремонт' ? `${s.active}` : ''}`)} onClick={this.getActiveCategory}>
                        ...
                        <ul className={cx(`${s.list} ${this.state.active === 'Ремонт' ? `${s.active}` : ''}`)}>
                            ...
                            <li className={s.item} onClick={this.setActiveMenu}><Link to='/other'>Ремонт другого устройства</Link></li>
                    ...
                    <div className={s.itemGroup}>
                        {this.props.location.pathname === '/iphone-sell' && <h3 className={s.title} onClick={this.setActiveMenu}><Link to='/basket'>Корзина</Link></h3>}
                    </div>
                </div>
            </nav>
        )
